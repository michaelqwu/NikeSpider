...
def main():
    ...
    baseurl = "https://www.nike.com/w?q="+urlName+"&vst="+urlName
    print(baseurl)
...
def getData(baseurl):
    ...
    for item in soup.find_all('div',class_="product-card__body"):
        ...
        # print(link)
        findName = re.compile(r'aria-describedby="(.*?)"')

        itemName = re.findall(findName,item)[0]
        # print(itemName)
        findImgLink = re.compile(r'<img alt=.*src="(.*?)"/>')
        # itemImg = re.findall(findImgLink,item)[0]
        # print(itemImg)
        findPrice = re.compile(r'="product-price">(.*?)</')
